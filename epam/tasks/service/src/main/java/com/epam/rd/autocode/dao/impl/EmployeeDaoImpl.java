...
public class EmployeeDaoImpl implements EmployeeDao {
    ...
    private Employee createEmployee(ResultSet resultSet) throws SQLException {
        return new Employee(BigInteger.valueOf(resultSet.getLong("id")),
                ...
                getMangerById(BigInteger.valueOf(resultSet.getLong("MANAGER"))),
                new DepartmentDaoImpl().getById(BigInteger.valueOf(resultSet.getLong("DEPARTMENT"))));
    }

    private Employee getMangerById(BigInteger id) {
        Employee employee = null;
        try (Connection connection = ConnectionSource.instance().createConnection();
             PreparedStatement statement = connection.prepareStatement(SELECT_EMPLOYEE_BY_ID)) {
    ...
    private Employee createManager(ResultSet resultSet) throws SQLException {
        return new Employee(BigInteger.valueOf(resultSet.getLong("id")),
                getEmployeeFullName(resultSet),
                Position.valueOf(resultSet.getString("POSITION").toUpperCase()),
                resultSet.getDate("HIREDATE").toLocalDate(),
    ...
    private void updateEmployee(Employee employee) {
        try (Connection connection = ConnectionSource.instance().createConnection();
            ...
            statement.setLong(7, employee.getManager().getId().longValue());
            statement.setLong(8, employee.getDepartment().getId().longValue());
    ...
    private void insertEmployee(Employee employee) {
        try (Connection connection = ConnectionSource.instance().createConnection();
            ...
            statement.setLong(8, employee.getManager().getId().longValue());
            statement.setLong(9, employee.getDepartment().getId().longValue());
            statement.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private Employee createEmployeeWithFullChain(ResultSet resultSet) throws SQLException {
        return new Employee(BigInteger.valueOf(resultSet.getLong("id")),
                getEmployeeFullName(resultSet),
                Position.valueOf(resultSet.getString("POSITION").toUpperCase()),
                resultSet.getDate("HIREDATE").toLocalDate(),
                resultSet.getBigDecimal("SALARY"),
                getByIdWithFullChain(BigInteger.valueOf(resultSet.getLong("MANAGER"))),
                new DepartmentDaoImpl().getById(BigInteger.valueOf(resultSet.getLong("DEPARTMENT"))));
    }
}
